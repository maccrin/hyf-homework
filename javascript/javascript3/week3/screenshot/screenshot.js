...
const userInputForm = document.getElementById("Loggedin-User-screenshot");
let userScreenShots;
const userScreenShotContainer = document.getElementById('user-screenshot-container');
...
const getBlogScreenShots = async () => {
    ...
    screenShotPost.forEach(eachShot => {
        if (eachShot.url) screenShotContainer.appendChild(createScreenshottDiv(eachShot))
    })
...
const getUsers =
    async () => {
        ...
        usersPost.forEach(user => {
            if (user.mail && user.password) userContainer.appendChild(createUserDiv(user));
        })
    }
